...
public class UserManager implements UserService {
    ...
    public DataResult<User> getByEmail(String email) {
        return new SuccessDataResult<User>(this.userDao.findByEmail(email),"Listed");
    }
    ...
    public DataResult<UserLoginReturnDto> login(UserLoginDto userLoginDto) {
        ...
        if(user==null){
            return new ErrorDataResult<UserLoginReturnDto>("You entered an incorrect email");
        }else if(!user.getPassword().equals(userLoginDto.getPassword())){
            return new ErrorDataResult<UserLoginReturnDto>("You entered an incorrect password");
        }else if(!user.isMailVerify()){
            return new ErrorDataResult<UserLoginReturnDto>("To login, you need to confirm your email.");
        }
        ...
        return new SuccessDataResult<UserLoginReturnDto>(userLoginReturnDto,"Signed in");
    }

    @Override
    public DataResult<List<User>> getVerifyedUsers() {
        return new SuccessDataResult<List<User>>(this.userDao.findByMailVerifyTrue(),"Data listed");
    }
}
